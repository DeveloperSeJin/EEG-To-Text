...
def get_config(case):
    if case == 'pretrain_autoencoder': 
        ...
        parser.add_argument('-epoch', '--num_epoch', type = int, help='num_epoch_step1', default = 20, required=True)
        ...
        parser.add_argument('-train_input', '--train_input', help='add noise' ,required=True)
        parser.add_argument('-cuda', '--cuda', help='specify cuda device name, e.g. cuda:0, cuda:1, etc', default = 'cuda:0')
        parser.add_argument('-ts', '--train_setting', help='diffusion trrain setting' ,required=False)
        args = vars(parser.parse_args())
