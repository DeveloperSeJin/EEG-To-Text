...
def train_autoencoder(dataloaders, device, model, pretrained, optimizer, scheduler, num_epochs=25, checkpoint_path_best = './checkpoints/decoding/best/temp_decoding.pt', checkpoint_path_last = './checkpoints/decoding/last/temp_decoding.pt'):
...
if __name__ == '__main__':
    ...
    save_name = f'pretrained_autoencoder_{task_name}_{batch_size}_{num_epoch}_{lr}'
    ...
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, autoencoder.parameters()), lr=lr, momentum=0.9)

    exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.1)

    print('=== start Step1 training ... ===')
    # print training layers
    show_require_grad_layers(autoencoder)
    # return best loss model from step1 training
    model = train_autoencoder(dataloaders, device, autoencoder, pretrained, optimizer, exp_lr_scheduler, num_epochs=num_epoch, checkpoint_path_best = output_checkpoint_name_best, checkpoint_path_last = output_checkpoint_name_last)